...
public:
  PatrolNavNode() : Node("patrol_nav_node") {
    ...
    declare_parameter<double>("obs_front_dist", 0.65);
    ...
    theft_pub_ = create_publisher<std_msgs::msg::Bool>("/theft_detected", 10);
    ...
    imu_sub_ = create_subscription<sensor_msgs::msg::Imu>(
        ...
        [this](const sensor_msgs::msg::Imu::SharedPtr msg){
            if (theft_detected_) return;
            
            double wx = msg->angular_velocity.x;
            double wy = msg->angular_velocity.y;
            double xy_magnitude = std::sqrt(wx*wx + wy*wy);
            
            if (xy_magnitude > 0.25) {
                if (imu_theft_start_.nanoseconds() == 0)
                    imu_theft_start_ = now();
                else if ((now() - imu_theft_start_).seconds() >= 0.3)
                    theft_detected_ = true;
            } else {
                imu_theft_start_ = rclcpp::Time(0, 0, RCL_ROS_TIME);
            }
        });
    ...
    nav_cmd_sub_ = create_subscription<std_msgs::msg::String>(
        ...
        [this](const std_msgs::msg::String::SharedPtr msg){
          if (msg->data == "once" || msg->data == "loop") {
              ...
              if (saved_target_wp_index_ != 0) {
                  target_wp_index_ = saved_target_wp_index_;
                  saved_target_wp_index_ = 0;
                  mode_ = Mode::PLAN;
                  if (msg->data == "once") nav_mode_ = NavMode::ONCE;
                  else nav_mode_ = NavMode::LOOP;

              } else if (!sent_to_start_ && stop_saved_target_ >= 0) {
                  int saved_seq_index = stop_saved_seq_index_;
                  int saved_target = stop_saved_target_;
                  loadWaypoints();

                  seq_index_ = stop_saved_seq_index_;
                  target_wp_index_ = stop_saved_target_;
                  stop_saved_target_ = -1;
                  stop_saved_seq_index_ = 0;
            ...
            imu_theft_start_ = rclcpp::Time(0, 0, RCL_ROS_TIME);
            last_theft_pub_time_ = rclcpp::Time(0, 0, RCL_ROS_TIME); 
            fire_stopped_ = false;
            fire_detected_ = false;
            stop_saved_target_ = target_wp_index_;
            stop_saved_seq_index_ = seq_index_;  
            publishZero();
            publishLED(7);
            RCLCPP_INFO(get_logger(), "정지 명령 수신");
          } else if (msg->data == "charging") {
            sent_to_start_ = true;
            RCLCPP_INFO(get_logger(), "충전 모드 → 시작 위치로 이동");
            nav_mode_ = NavMode::CHARGE;
            target_wp_index_ = seq_[0];
            robot_stopped_ = false;
            person_detected_ = false;
            buzzer_on_ = false;
            charge_goto_arrived_ = false;
            stop_saved_target_ = -1;
            stop_saved_seq_index_ = 0;
            publishLED(8);
            ...
            if (std::fabs(dyaw) >= trig) {
              ...
              nav_mode_ = NavMode::MANUAL;
              publishZero();
              publishLED(7);
          } else {
            std::string label = msg->data;
            for (int i = 0; i < (int)waypoints_.size(); i++) {
                if (waypoints_[i].label == label) {
                    RCLCPP_INFO(get_logger(), "구역 이동 → %s", label.c_str());
                    nav_mode_ = NavMode::GOTO;
                    target_wp_index_ = i;
                    stop_saved_target_ = -1;
                    stop_saved_seq_index_ = 0;
                    if (i == (int)seq_[0]) sent_to_start_ = true;
                    robot_stopped_ = false;
                    person_detected_ = false;
                    buzzer_on_ = false;
                    charge_goto_arrived_ = false;
            ...
            RCLCPP_WARN(get_logger(), "웨이포인트 없음: %s", label.c_str());
          }
        });
    ...
    fire_sub_ = create_subscription<std_msgs::msg::Bool>("/fire_detected", 10,
        [this](const std_msgs::msg::Bool::SharedPtr msg){
            if (nav_mode_ == NavMode::IDLE) return;
...
private:
  ...
  void loadWaypoints() {
    ...
    for (int i = N-2; i >= 0; i--) seq_.push_back(i);
  ...
  void advanceWaypoint() {
    ...
    if (nav_mode_ == NavMode::ONCE && seq_index_ == static_cast<int>(seq_.size()) - 1) {
        nav_once_done_ = true;
        nav_mode_ = NavMode::IDLE;
        stop_saved_target_ = -1;
        stop_saved_seq_index_ = 0;
  ...
  void triggerSectorFallback() {
    ...
    for (int i = 0; i < N; i++) base_seq.push_back(i);
    for (int i = N-2; i >= 1; i--) base_seq.push_back(i);

    // 막힌 웨이포인트가 기본 seq에서 처음 등장하는 위치
    int base_blocked_idx = 0;
    for (int i = 0; i < (int)base_seq.size(); i++) {
        if (base_seq[i] == target_wp_index_) {
            base_blocked_idx = i;
            break;
        }
    }
    ...
    for (int i = base_blocked_idx - 1; i >= 0; i--)
        new_seq.push_back(base_seq[i]);
    ...
    for (int i = 1; i < N; i++) new_seq.push_back(i);
    ...
    seq_index_ = 0;
    target_wp_index_ = seq_[0];
    std::string seq_str = "새 seq: ";
    for (int i : seq_) seq_str += std::to_string(i) + " ";
    RCLCPP_WARN(get_logger(), "%s", seq_str.c_str());
    astar_fail_count_ = 0;
    obs_blocked_start_ = rclcpp::Time(0, 0, RCL_ROS_TIME);
    global_path_.poses.clear();
    mode_ = Mode::PLAN;
    RCLCPP_WARN(get_logger(), "섹터 막힘 → 역순 복귀 후 B섹터 방문 후 전체 순회");
  }
  ...
  void returnToPlanWithRotate() {
    ...
    double trig=get_parameter("rotate_trigger_deg").as_double()*M_PI/180.0;
    std_msgs::msg::Bool fm; fm.data=false; follow_pub_->publish(fm);
    if(std::fabs(dyaw)>=trig){rotate_target_yaw_=heading;rotate_settle_count_=0; rotate_is_replan_ = true; mode_=Mode::ROTATE;}
  ...
  void onTimer() {
    if (theft_detected_) {
      ...
      if (last_theft_pub_time_.nanoseconds() == 0 ||
        (now() - last_theft_pub_time_).seconds() >= 5.0) {
        std_msgs::msg::Bool theft_msg;
        theft_msg.data = true;
        theft_pub_->publish(theft_msg);
        last_theft_pub_time_ = now();
      }
      return;
    } else {
    ...
    if (battery_percentage_ < BATTERY_LOW_THRESHOLD &&
        nav_mode_ != NavMode::CHARGE &&
        nav_mode_ != NavMode::IDLE) {
        RCLCPP_WARN(get_logger(), "배터리 부족 (%.1f%%) → 시작 위치로 복귀", battery_percentage_);
        nav_mode_ = NavMode::CHARGE;
        target_wp_index_ = seq_[0];
        robot_stopped_ = false;
        person_detected_ = false;
    ...
    if (fire_detected_ && !fire_stopped_) {
        if (saved_target_wp_index_ == 0)
            saved_target_wp_index_ = target_wp_index_;
        
        if (fire_stop_time_.nanoseconds() == 0 ||
            (now() - fire_stop_time_).seconds() >= 1.0) {
            playSound(2);
            fire_stop_time_ = now();
        }
        ...
        if (front_dist <= 1.0) {
            ...
            if (std::fabs(pan_error) < 0.05) pan_error = 0.0;
            publishCmd(0.15, -0.8 * pan_error);
            publishLED(6);
        }
        return;
    }
    ...
    if (fire_stopped_) {
        publishZero();
        publishLED(6);
        if ((now() - fire_stop_time_).seconds() >= 1.0) {
            playSound(2);
            fire_stop_time_ = now();
        }
        return;
    ...
    if (mode_!=Mode::ROTATE&&mode_!=Mode::FOLLOW&&mode_!=Mode::SEARCH&&mode_!=Mode::SEARCH_BACKUP&&reachedWaypoint(tgt)){
      if (nav_mode_ == NavMode::CHARGE || nav_mode_ == NavMode::GOTO) {
        ...
        if (std::fabs(dyaw) >= tol) {
            charge_goto_arrived_ = true;
            rotate_target_yaw_ = target_yaw;
            rotate_settle_count_ = 0;
            rotate_is_replan_ = false;
            mode_ = Mode::ROTATE;
            publishZero();
            return;
        }
      ...
      double dyaw=normAngle(heading-pose_yaw_);
      double trig=get_parameter("rotate_trigger_deg").as_double()*M_PI/180.0;
      if(std::fabs(dyaw)>=trig){rotate_target_yaw_=heading;rotate_settle_count_=0; rotate_is_replan_ = false; mode_=Mode::ROTATE;}
      else mode_=Mode::PLAN;
      publishZero(); return;
    }
    ...
    if (mode_==Mode::ROTATE){
      obs_blocked_start_ = rclcpp::Time(0, 0, RCL_ROS_TIME);
      if (nav_mode_ != NavMode::CHARGE && nav_mode_ != NavMode::GOTO) {
        if (rotate_is_replan_) publishLED(3);
        else publishLED(5);
      }
    ...
    // ── TRACK 모드 ───────────────────────────────────────────────────
    if (mode_==Mode::TRACK){
      if (astar_fail_count_ == 0) publishLED(5);
      ...
      if(person_detected_ && !ignore_person && person_bbox_z_ >= 0.10 && front_dist <= 2.0){
        std_msgs::msg::Bool sm; sm.data=false; stopped_pub_->publish(sm);
    ...
    if (mode_ == Mode::FOLLOW) {
      ...
      if (robot_stopped_ || (person_really_detected && front_dist_follow <= 1.0)) {
        ObsInfo obs = analyzeScan();
        if (obs.front_blocked && !person_really_detected) {
    ...
    if(!ok_pp){
      ObsInfo obs_pp = analyzeScan();
      if (obs_pp.front_blocked) {
        bool gl=(obs_pp.left_clear>=obs_pp.right_clear); double sign=gl?+1.0:-1.0;
        double wg=get_parameter("avoid_w_gain").as_double();
        double wa=std::clamp(sign*wg*(1.0/std::max(0.25,std::min(obs_pp.left_clear,obs_pp.right_clear))),
                             -get_parameter("w_max").as_double(),get_parameter("w_max").as_double());
        publishCmd(get_parameter("v_min").as_double(), wa);
        return;
      }
      ...
      double target_yaw=std::atan2(ty-pose_y_,tx-pose_x_);
      rotate_target_yaw_ = target_yaw;
      rotate_settle_count_ = 0;
      rotate_is_replan_ = true;
      publishLED(3);
      mode_ = Mode::ROTATE;
      publishZero();
      return;
...
private:
  ...
  enum class NavMode { IDLE, LOOP, ONCE, CHARGE, GOTO, MANUAL };
  ...
  static constexpr int ASTAR_FAIL_MAX = 5;
  rclcpp::Time obs_blocked_start_{0, 0, RCL_ROS_TIME};
  static constexpr double OBS_BLOCKED_TIMEOUT = 1.0;
  ...
  bool charge_goto_arrived_{false};
  bool rotate_is_replan_{false};


  rclcpp::Publisher<std_msgs::msg::Int32>::SharedPtr led_pub_;

  rclcpp::Time imu_theft_start_{0, 0, RCL_ROS_TIME};

  bool fire_detected_{false};
  bool fire_stopped_{false};
  int saved_target_wp_index_{0};
  rclcpp::Time fire_stop_time_{0, 0, RCL_ROS_TIME};
  rclcpp::Subscription<std_msgs::msg::Bool>::SharedPtr fire_sub_;

  double battery_percentage_{100.0};
  rclcpp::Subscription<sensor_msgs::msg::BatteryState>::SharedPtr battery_sub_;
  static constexpr double BATTERY_LOW_THRESHOLD = 20.0;

  rclcpp::Publisher<std_msgs::msg::Bool>::SharedPtr theft_pub_;
  rclcpp::Time last_theft_pub_time_{0, 0, RCL_ROS_TIME};

  int stop_saved_target_{-1};
  int stop_saved_seq_index_{0};
  bool sent_to_start_{false};
};
