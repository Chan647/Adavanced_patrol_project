...
public:
  PatrolNavNode() : Node("patrol_nav_node") {
    ...
    bbox_sub_ = create_subscription<geometry_msgs::msg::Vector3>("/person_bbox", 10,
    ...
    imu_sub_ = create_subscription<sensor_msgs::msg::Imu>(
        ...
        [this](const sensor_msgs::msg::Imu::SharedPtr msg){
          ...
          if (w > 1.00) {
              theft_detected_ = true;
          } else {
              theft_detected_ = false;
          }
      });
    ...
    nav_cmd_sub_ = create_subscription<std_msgs::msg::String>(
        "/cpp_nav_command", 10,
        [this](const std_msgs::msg::String::SharedPtr msg){
          if (msg->data == "once" || msg->data == "loop") {
            loadWaypoints();
            ...
            if (msg->data == "once") {
              ...
              RCLCPP_INFO(get_logger(), "1회 주행 시작");
            } else {
              nav_mode_ = NavMode::LOOP;
              RCLCPP_INFO(get_logger(), "무한반복 주행 시작");
            }
            nav_once_done_ = false;
            seq_index_ = 0; target_wp_index_ = seq_[0];
            mode_ = Mode::PLAN;
    ...
    sound_client_ = create_client<turtlebot3_msgs::srv::Sound>("sound");
  }

private:
  enum class Mode { WAIT, PLAN, ROTATE, TRACK, AVOID, FOLLOW };
  ...
  double getFrontPersonDist() const {
    ...
    double fov = 30.0 * M_PI / 180.0;  // 정면 ±30도
  ...
  void advanceWaypoint() {
    ...
    if (nav_mode_ == NavMode::ONCE && target_wp_index_ == 0 && seq_index_ != 0) {
      nav_once_done_ = true;
      nav_mode_ = NavMode::IDLE;
      publishZero();
  ...
  void planToTarget() {
    ...
    if (!aStarPlan(pose_x_,pose_y_,tgt.x,tgt.y,path)){
        ...
        astar_fail_count_++;
        if (astar_fail_count_ >= ASTAR_FAIL_MAX) {
            triggerSectorFallback();
        }
        return;
    }
    astar_fail_count_ = 0;
  ...
  void resetFollowState() {
    searching_person_=false; search_back_=false; search_direction_fixed_=false;
    search_full_rotation_=false; search_accumulated_yaw_=0.0; search_last_yaw_=0.0;
    person_lost_time_=rclcpp::Time(0); robot_stopped_=false;
    stop_settled_=false; stop_settled_time_=rclcpp::Time(0);
    stop_time_=rclcpp::Time(0); face_timeout_=false;
    last_buzzer_time_=rclcpp::Time(0); person_detect_confirm_=0;
  ...
  bool isSearchDone() {
    ...
    if (search_full_rotation_) {
        return (search_accumulated_yaw_ >= 2.0*M_PI - 0.15);
    }

    if (search_accumulated_yaw_ < 0.3) return false;
    return (std::fabs(normAngle(search_target_yaw_-pose_yaw_)) <= 0.15);
  }

  void startSearchRotation() {
    search_direction_fixed_=true; searching_person_=true; search_start_time_=now();
    search_full_rotation_=false; search_accumulated_yaw_=0.0; search_last_yaw_=pose_yaw_;

    double pan_offset = pan_angle_ - 80.0;
    if (std::fabs(pan_offset) > 10.0) {
      search_w_ = (pan_offset > 0) ? 0.4 : -0.4;
      search_back_ = false;
      search_target_yaw_ = normAngle(pose_yaw_ + (search_w_ > 0 ? M_PI/2.0 : -M_PI/2.0));
      RCLCPP_INFO(get_logger(), "pan 방향으로 탐색: pan=%.1f w=%.2f", pan_angle_, search_w_);
      return;
    }

    if (!stopped_scan_ranges_.empty()&&stopped_scan_ranges_.size()==scan_.ranges.size()) {
      double ld=0.0,rd=0.0,bd=0.0; int lc=0,rc=0,bc=0;
      int n=static_cast<int>(scan_.ranges.size());
      for (int i=0;i<n;i++){
        double ang=scan_.angle_min+i*scan_.angle_increment;
        while(ang> M_PI) ang-=2.0*M_PI; while(ang<-M_PI) ang+=2.0*M_PI;
        if(!std::isfinite(stopped_scan_ranges_[i])||!std::isfinite(scan_.ranges[i])) continue;
        double delta=stopped_scan_ranges_[i]-scan_.ranges[i];
        if(delta<SCAN_DELTA_THRESHOLD) continue;
        if(std::fabs(ang)>M_PI/2.0){bd+=delta;bc++;}
        else if(ang>0.0){ld+=delta;lc++;}
        else if(ang<0.0){rd+=delta;rc++;}
      }
      double la=(lc>0)?ld/lc:0.0, ra=(rc>0)?rd/rc:0.0, ba=(bc>0)?bd/bc:0.0;
      if (ba>0.0&&ba>=la&&ba>=ra){
        search_w_=(last_person_bbox_x_>=0)?-0.4:0.4; search_back_=true;
        search_target_yaw_=normAngle(pose_yaw_+(search_w_>0?M_PI:-M_PI));
        RCLCPP_INFO(get_logger(),"뒤쪽 스캔 변화 → 180도 회전");
      } else if (la>=ra&&la>0.0){
        search_w_=0.4; search_back_=false;
        search_target_yaw_=normAngle(pose_yaw_+M_PI/2.0);
        RCLCPP_INFO(get_logger(),"왼쪽 스캔 변화 (delta=%.2f)",la);
      } else if (ra>la&&ra>0.0){
        search_w_=-0.4; search_back_=false;
        search_target_yaw_=normAngle(pose_yaw_-M_PI/2.0);
        RCLCPP_INFO(get_logger(),"오른쪽 스캔 변화 (delta=%.2f)",ra);
      } else {
        search_w_=0.4; search_back_=true; search_full_rotation_=true;
        RCLCPP_INFO(get_logger(),"스캔 변화 없음 → 한 바퀴 탐색");
      }
      return;
    }
    search_w_=0.4; search_back_=true; search_full_rotation_=true;
    RCLCPP_INFO(get_logger(),"스캔 기준 없음 → 한 바퀴 탐색");
  }
  ...
  void onTimer() {
    ...
    if (mode_!=Mode::ROTATE&&mode_!=Mode::FOLLOW&&reachedWaypoint(tgt)){
      advanceWaypoint();
    ...
    if (mode_==Mode::ROTATE){
      ...
      if(std::fabs(dyaw)<=tol){
        ...
        if(++rotate_settle_count_>=5){rotate_settle_count_=0;mode_=Mode::PLAN;}
        return;
      }
    ...
    if (global_path_.poses.size()<2&&mode_!=Mode::FOLLOW){
    ...
    if (mode_==Mode::TRACK){
        ...
        if(obs.front_blocked){
            if (obs_blocked_start_.nanoseconds() == 0)
                obs_blocked_start_ = now();
        ...
        RCLCPP_INFO_THROTTLE(get_logger(), *get_clock(), 1000, "person_bbox_z=%.3f front_dist=%.2f", person_bbox_z_, front_dist);
        if(person_detected_ && !ignore_person && person_bbox_z_ >= 0.10 && front_dist <= 2.5){
            ...
            if(person_detect_count_>=PERSON_DETECT_THRESHOLD){
                ...
                person_lost_time_=rclcpp::Time(0); person_detect_confirm_=0; mode_=Mode::FOLLOW;
    ...
    if (mode_ == Mode::FOLLOW) {
      ...
      // ── 케이스 1: 사람 소실 (이동 중) ──────────────────────────────
      if (!person_really_detected && !robot_stopped_) {
          ...
          if (!search_direction_fixed_) {
              publishZero();
              search_accumulated_yaw_ = 0.0;
              search_last_yaw_ = pose_yaw_;
              startSearchRotation();
              buzzer_on_ = true;
              RCLCPP_WARN(get_logger(), "케이스1 탐색 시작 → 부저 ON");
          }

          bool search_done = isSearchDone();

          if (!search_done) {
              if (person_detect_confirm_ >= 2) {
                  RCLCPP_INFO(get_logger(), "케이스1 탐색 중 재감지 → FOLLOW 복귀");
                  search_direction_fixed_ = false; searching_person_ = false; search_back_ = false;
                  search_full_rotation_ = false; search_accumulated_yaw_ = 0.0;
          ...
          if (person_detect_confirm_ >= 2) {
              RCLCPP_INFO(get_logger(), "탐색 완료 후 재감지 → FOLLOW 복귀");
              search_direction_fixed_ = false; searching_person_ = false; search_back_ = false;
              search_full_rotation_ = false; search_accumulated_yaw_ = 0.0;
              person_lost_time_ = rclcpp::Time(0);
              return;
          ...
          RCLCPP_INFO(get_logger(), "탐색 완료, 사람 없음 → 경로 재설정");
          buzzer_on_ = false; last_buzzer_time_ = rclcpp::Time(0); face_success_done_ = false;
          search_direction_fixed_ = false; searching_person_ = false; search_back_ = false;
          search_full_rotation_ = false; search_accumulated_yaw_ = 0.0;
          person_lost_time_ = rclcpp::Time(0);
      ...
      if (!person_really_detected && robot_stopped_) {
          ...
          if ((now() - person_lost_time_).seconds() < 0.5) {
              publishZero();
              std_msgs::msg::Bool sm; sm.data = true; stopped_pub_->publish(sm);
              if (!buzzer_on_) {
                  buzzer_on_ = true; playSound(2);
                  RCLCPP_WARN(get_logger(), "정지 중 사람 사라짐 → 즉시 부저 ON");
              }
              return;
          }
          ...
          if (!buzzer_on_) {
              buzzer_on_ = true; playSound(2);
              RCLCPP_WARN(get_logger(), "정지 중 사람 사라짐 → 부저 ON");
          }
          ...
          if (!search_direction_fixed_) {
              ...
              if (!stopped_scan_ranges_.empty() && have_scan_) {
                  ...
                  for (size_t i = 0; i < stopped_scan_ranges_.size(); i++) {
                      ...
                      if (diff < best_diff) {
                          best_diff = diff;
                          person_dist = stopped_scan_ranges_[i];
                      }
                  }
              }

              if (person_dist > 1.5) {
                  std_msgs::msg::Bool sm2; sm2.data = false; stopped_pub_->publish(sm2);
                  std_msgs::msg::Bool fm; fm.data = true; follow_pub_->publish(fm);
                  RCLCPP_INFO(get_logger(), "사람 멀리 있음(%.2fm) → tilt 35으로 탐색", person_dist);
              } else {
                  RCLCPP_INFO(get_logger(), "사람 가까이 있음(%.2fm) → tilt 15 유지 탐색", person_dist);
              }
          }
          ...
          if (person_detect_confirm_ >= 2) {
            ...
            if (cur_dist <= 1.2) {
                RCLCPP_INFO(get_logger(), "탐색 중 재감지(가까움) → 바로 정지");
                resetFollowState();
                robot_stopped_ = true;
                buzzer_on_ = true;
                std_msgs::msg::Bool sm; sm.data = true; stopped_pub_->publish(sm);
                ...
                RCLCPP_INFO(get_logger(), "탐색 중 재감지(멀리) → FOLLOW 추종");
                resetFollowState();
                buzzer_on_ = true;
                std_msgs::msg::Bool fm; fm.data = true; follow_pub_->publish(fm);
            }
          ...
          RCLCPP_INFO(get_logger(), "탐색 완료, 사람 없음 → 부저 끄고 경로 재설정");
          buzzer_on_ = false; last_buzzer_time_ = rclcpp::Time(0);
          face_timeout_ = false; face_recognized_ = false; face_success_done_ = false;
          resetFollowState();
          returnToPlanWithRotate();
          return;
      }

      // ── 케이스 3: 사람 감지됨 → 정지 or 추종 ───────────────────────
      if (robot_stopped_ || front_dist_follow <= 1.2) {
          ObsInfo obs = analyzeScan();
          if (obs.front_blocked) {
          ...
          if (front_dist_follow > 1.5 && robot_stopped_ && person_really_detected) {
              std_msgs::msg::Bool sm; sm.data = false; stopped_pub_->publish(sm);
          ...
          if (face_recognized_) {
              ...
              advanceWaypoint(); returnToPlanWithRotate();
          }
      } else {
...
private:
  ...
  double search_w_{0.4}; bool search_back_{false}; double search_target_yaw_{0.0};
  bool   search_full_rotation_{false};
  double search_accumulated_yaw_{0.0};
  double search_last_yaw_{0.0};
  std::vector<float> stopped_scan_ranges_; double last_person_bbox_x_{0.0};
  bool searching_person_{false},search_direction_fixed_{false};
  int  person_detect_confirm_{0}; rclcpp::Time search_start_time_;
  static constexpr double SCAN_DELTA_THRESHOLD=0.3;

  enum class NavMode { IDLE, LOOP, ONCE } nav_mode_{NavMode::IDLE};
  ...
  static constexpr int ASTAR_FAIL_MAX = 3;
