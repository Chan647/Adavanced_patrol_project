...
const float CONF_THRESHOLD      = 0.63f; 
const float FIRE_CONF_THRESHOLD = 0.88f; 
...
public:
          ...
          last_person_time_(rclcpp::Time(0))
    {
        ...
        bbox_pub_      = this->create_publisher<geometry_msgs::msg::Vector3>("/person_bbox", rclcpp::QoS(1).best_effort());
        ...
        // ======================================================================
        // 💡 [웹 연동 파트 1] 통신 채널 설정 (생존 펄스 및 ON/OFF 토픽)
        // ======================================================================
        ...
        toggle_sub_ = this->create_subscription<std_msgs::msg::Bool>(
            ...
            [this](const std_msgs::msg::Bool::SharedPtr msg) {
                is_detection_enabled_ = msg->data;
                RCLCPP_INFO(this->get_logger(), "🔥 [웹 명령 수신] 객체 감지 상태: %s", is_detection_enabled_ ? "ON" : "OFF");
            });
        ...
        RCLCPP_INFO(this->get_logger(), "✅ Detection Node 시작 (웹 연동 준비 완료)");
    }
...
private:
        ...
        const sensor_msgs::msg::CompressedImage::SharedPtr msg)
    {
        ...
        if (!realDetected) {
            ...
            if (faceFrameCount_++ % 20 == 0) {
                if (target_locked_) {
                    ...
                    expanded.width  = std::min(imgW - expanded.x,
                                               expanded.width  + 2 * marginX);
                    expanded.height = std::min(imgH - expanded.y,
                                               expanded.height + marginY_top + marginY_bottom);
        ...
        for (const auto& det : lastFaceDets_) {
            if (det.name == "Unknown") continue;  // ★ Unknown은 박스/라벨 표시 안 함
        ...
        if (personDetected && !lastPersonDets_.empty()) {
            if (!target_locked_) {
                ...
                if (det.box.area() >= imgW * imgH * 0.05) {
                    target_box_    = det.box;
                    target_locked_ = true;
                }
            }
        ...
        bool realDet = !lastPersonDets_.empty();
        if (realDet) {
    ...
    // 💡 [웹 연동 파트 3] 변수 선언부
    bool is_detection_enabled_{false}; // 시작 시 기본 상태: OFF
